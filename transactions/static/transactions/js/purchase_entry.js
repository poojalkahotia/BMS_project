...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const dataScript = document.getElementById('purchase-data');
    const initialPurchaseData = dataScript ? JSON.parse(dataScript.textContent) : null;

    if (initialPurchaseData && initialPurchaseData.isEditing) {
        initializeEditMode(initialPurchaseData);
    }
});
...
function addOrUpdateItem() {
    ...
    if (currentRowIndex >= 0) {
        // Update existing row (The user clicked a row to edit it)
...
function editRow(row) {
    ...
    // (Handled by checking event target in deleteRow, but mainly we want row click)
...
function savePurchase() {
    ...
    const payload = {
        inv_no: invNo, // Backend will likely ignore or regenerate if "New"
        inv_date: invDate,
        ...
        // Party Snapshot (Optional, backend can fetch or we send displayed values)
    ...
    .then(data => {
        if (data.status === 'success') {
            alert(data.message);
            window.location.href = '/purchase/list/'; // Reset form for next entry
        } else {
